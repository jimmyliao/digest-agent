...
export function initDb(): void {
  ...
  // Seed RSS sources if empty (matching config/settings.yaml)
...
export function insertArticle(article: Article): number {
  const db = getDb();
...
export function getArticlesByStatus(status: string = 'all', limit: number = 50): Article[] {
  const db = getDb();
  if (status === 'all') {
...
export function updateArticleStatus(id: number, status: string, summary?: string): void {
  const db = getDb();
...
// Task records
export function createTask(taskId: string, taskType: string, total: number = 0): void {
  const db = getDb();
  db.run(
...
export function updateTask(taskId: string, status: string, completed: number, resultJson?: string, errorJson?: string): void {
  const db = getDb();
  db.run(
...
export function getRecentTasks(limit: number = 20): unknown[] {
